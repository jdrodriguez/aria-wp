...
(function() {
    ...
    class AriaChat {
        constructor(config) {
            this.config = Object.assign({
                ...
                requireEmail: false,
                gdprEnabled: false,
                apiEndpoint: window.ariaChat ? window.ariaChat.ajaxUrl : '/wp-admin/admin-ajax.php',
        ...
        createChatWindow() {
            ...
            // Cache DOM elements
            this.messagesContainer = this.window.querySelector('.aria-messages-container');
            this.messageInput = this.window.querySelector('.aria-message-input');
            this.chatForm = this.window.querySelector('.aria-chat-form');
            this.typingIndicator = this.window.querySelector('.aria-typing-indicator');
        }
        ...
        bindEvents() {
            ...
            // Window controls
            this.window.querySelector('.aria-close-btn').addEventListener('click', () => this.close());
        ...
        applyStyles() {
            ...
            const pluginUrl = window.ariaChat && window.ariaChat.pluginUrl ? window.ariaChat.pluginUrl : '/wp-content/plugins/aria/';
            style.textContent = `
                .aria-chat-widget {
        ...
        loadSession() {
            ...
            if (savedData) {
                try {
                    ...
                    if (this.sessionData.messages && this.sessionData.timestamp) {
                    ...
                    console.error('Failed to load Aria session:', e);
                }
            }
        }
        ...
        saveSession() {
            ...
            localStorage.setItem('aria_session_' + sessionId, JSON.stringify(this.sessionData));
        }
        ...
        open() {
            ...
            if (!this.conversationId && (!this.sessionData.name || !this.sessionData.email)) {
        ...
        close() {
            ...
            if (this.config.enableAnimations) {
                ...
                setTimeout(() => {
                    this.window.style.display = 'none';
                    this.window.classList.remove('aria-window-open', 'aria-window-closing');
        ...
        sendInitialGreeting() {
            ...
            this.sendRequest('aria_start_conversation', {
                ...
                if (response.success) {
                    ...
                    if (response.data.session_id) {
                        sessionStorage.setItem('aria_session_id', response.data.session_id);
                    }
        ...
        showUserInfoForm() {
            ...
            const strings = window.ariaChat && window.ariaChat.strings ? window.ariaChat.strings : {};
            const welcomeMessage = strings.welcomeMessage || 'Welcome! Please introduce yourself so I can assist you better.';
            const enterName = strings.enterName || 'Your name';
            const enterEmail = strings.enterEmail || 'Your email';
            const enterPhone = strings.enterPhone || 'Your phone number (optional)';
            ...
            form.addEventListener('submit', (e) => this.handleUserInfoSubmit(e));
        ...
        handleUserInfoSubmit(e) {
            ...
            const form = e.target;
            const name = form.querySelector('#aria-user-name').value.trim();
            const email = form.querySelector('#aria-user-email').value.trim();
            const phone = form.querySelector('#aria-user-phone').value.trim();
            const message = form.querySelector('#aria-user-message').value.trim();
            ...
            if (!emailRegex.test(email)) {
                this.addMessage(window.ariaChat ? window.ariaChat.strings.invalidEmail : 'Please enter a valid email address.', 'aria');
                return;
            }
        ...
        requestEmail(pendingMessage = null) {
            this.addMessage(window.ariaChat ? window.ariaChat.strings.emailRequired : 'Please provide your email to continue.', 'aria');
            if (pendingMessage) {
                this.pendingMessage = pendingMessage;
            }
            this.awaitingEmail = true;
            this.messageInput.setAttribute('type', 'email');
            this.messageInput.setAttribute('placeholder', window.ariaChat && window.ariaChat.strings.enterEmail ? window.ariaChat.strings.enterEmail : 'Your email');
        }
        ...
        addMessage(text, sender = 'user', options = {}) {
            ...
            if (sender === 'aria' && this.config.showAvatar) {
                const logoClass = this.currentTheme === 'dark' ? 'aria-logo-dark' : 'aria-logo-light';
                messageHtml += `
            ...
            if (this.config.enableAnimations) {
                ...
                setTimeout(() => messageEl.classList.remove('aria-message-new'), 10);
            }
            ...
            // Play sound
            if (this.config.enableSound && sender === 'aria' && !options.skipSound) {
        ...
        sendMessage(message) {
            ...
            if (this.awaitingEmail) {
                ...
                if (emailRegex.test(message)) {
                    ...
                    this.addMessage(window.ariaChat ? window.ariaChat.strings.thankYouEmail : 'Thank you! How can I help you today?', 'aria');
                    
                    // Reset input type
                    this.messageInput.setAttribute('type', 'text');
                    this.messageInput.setAttribute('placeholder', window.ariaChat ? window.ariaChat.strings.typeMessage : 'Type your message...');
            ...
            // Send to server
            this.sendRequest('aria_send_message', {
                ...
                if (response.success) {
                    ...
                    this.addMessage(response.data?.message || (window.ariaChat ? window.ariaChat.strings.errorMessage : null) || 'Something went wrong. Please try again.', 'aria');
                }
            });
        }
         ...
         */
        requestHumanHandoff() {
            this.addMessage(window.ariaChat ? window.ariaChat.strings.connectingHuman : 'Connecting you with a human...', 'system');
        ...
        showFeedbackRequest() {
            ...
            feedbackMessage.querySelectorAll('.aria-feedback-btn').forEach(btn => {
                btn.addEventListener('click', (e) => this.submitFeedback(e.target.dataset.rating));
            });
        ...
        submitFeedback(rating) {
            this.sendRequest('aria_submit_feedback', {
                conversation_id: this.conversationId,
                rating: rating
            }, (response) => {
                if (response.success) {
                    // Replace feedback buttons with thank you
                    const feedbackEl = this.messagesContainer.querySelector('.aria-feedback-request');
                    if (feedbackEl) {
        ...
        sendRequest(action, data, callback) {
            ...
            if (!window.ariaChat || !window.ariaChat.nonce) {
                console.error('AriaChat: Missing nonce. ariaChat object:', window.ariaChat);
                callback({ 
                    success: false, 
                    data: { 
                        message: 'Configuration error: Missing security token. Please refresh the page.' 
                    } 
                });
                return;
            }
        ...
        applyButtonIcon() {
            ...
            const iconContainer = this.button.querySelector('.aria-button-icon');
            ...
            iconContainer.innerHTML = icons[this.config.buttonIcon] || icons['chat-bubble'];
        }
        ...
        setTheme(theme) {
            ...
            this.container.classList.remove('aria-theme-light', 'aria-theme-dark');
        ...
        updateLogos() {
            ...
            const messageAvatars = this.container.querySelectorAll('.aria-avatar-aria');
        ...
        getMessageHistory() {
            const messages = [];
            this.messagesContainer.querySelectorAll('.aria-message').forEach(msg => {
                ...
                if (textElement) {
                    const sender = msg.classList.contains('aria-message-user') ? 'user' : 'aria';
        ...
        trackEvent(eventName, data = {}) {
            ...
            if (this.config.enableAnalytics) {
                this.sendRequest('aria_track_event', {
                    event: eventName,
                    conversation_id: this.conversationId,
                    ...data
                }, () => {
         ...
         */
        endConversation() {
            this.addMessage(window.ariaChat ? (window.ariaChat.strings.conversationEnded || 'Thank you for chatting with me. Have a great day!') : 'Thank you for chatting with me. Have a great day!', 'aria');
        ...
        formatMessageText(text) {
            ...
            const messages = this.messagesContainer.querySelectorAll('.aria-message');
            let lastUserMessage = '';
            for (let i = messages.length - 1; i >= 0; i--) {
                if (messages[i].classList.contains('aria-message-user')) {
                    const textEl = messages[i].querySelector('.aria-message-text');
                    if (textEl) {
            ...
            while ((match = markdownLinkRegex.exec(text)) !== null) {
                links.push({
                    ...
                    url: match[2],
                    fullMatch: match[0],
                    context: { userMessage: lastUserMessage, responseText: text.toLowerCase() }
                });
            }
            ...
            while ((match = urlRegex.exec(text)) !== null) {
                ...
                for (const link of links) {
                    if (link.fullMatch.includes(match[0])) {
                        isMarkdownLink = true;
                        break;
                    }
                }
                if (!isMarkdownLink) {
                    ...
                    links.push({
                        text: this.getLinkText(href, lastUserMessage, text.toLowerCase()),
                        url: href,
                        fullMatch: url,
                        context: { userMessage: lastUserMessage, responseText: text.toLowerCase() }
                    });
        ...
        getLinkText(url, userMessage = '', responseText = '') {
            try {
                ...
                if (lowerUserMessage.includes('reservation') || lowerUserMessage.includes('book') || lowerUserMessage.includes('table')) {
                    return 'Make a Reservation';
                }
                if (lowerUserMessage.includes('hour') || lowerUserMessage.includes('open') || lowerUserMessage.includes('close')) {
                    return 'View Hours';
                }
                if (lowerUserMessage.includes('menu') || lowerUserMessage.includes('food') || lowerUserMessage.includes('dish')) {
                    return 'View Menu';
                }
                if (lowerUserMessage.includes('direction') || lowerUserMessage.includes('location') || lowerUserMessage.includes('where')) {
                    return 'Get Directions';
                }
                if (lowerUserMessage.includes('contact') || lowerUserMessage.includes('phone') || lowerUserMessage.includes('email')) {
                    return 'Contact Us';
                }
                if (lowerUserMessage.includes('work') || lowerUserMessage.includes('job') || lowerUserMessage.includes('career') || lowerUserMessage.includes('hiring')) {
                    return 'Join Our Team';
                }
                
                // Check URL patterns as fallback
                if (pathname.includes('careers') || pathname.includes('jobs') || pathname.includes('employment')) {
                    return 'Join Our Team';
                }
                if (pathname.includes('menu')) {
                    return 'View Menu';
                }
                if (pathname.includes('reservation') || pathname.includes('booking')) {
                    return 'Make a Reservation';
                }
                ...
                if (pathname.includes('location') || pathname.includes('directions')) {
                    return 'Get Directions';
                }
    ...
    class AriaEmbedChat {
        ...
        cacheElements() {
            // Form elements
            this.formView = this.container.querySelector('.aria-embed-form-view');
            this.intakeForm = this.container.querySelector('.aria-embed-intake-form');
            ...
            this.messageInput = this.container.querySelector('#aria-embed-message');
            
            // Chat elements
            this.chatView = this.container.querySelector('.aria-embed-chat-view');
            this.messagesContainer = this.container.querySelector('.aria-embed-messages-container');
            this.chatForm = this.container.querySelector('.aria-embed-chat-form');
            this.chatInput = this.container.querySelector('.aria-embed-message-input');
            this.typingIndicator = this.container.querySelector('.aria-typing-indicator');
        ...
        bindEvents() {
            ...
            this.intakeForm.addEventListener('submit', (e) => this.handleIntakeSubmit(e));
            ...
            // Close button
            if (this.closeButton) {
                this.closeButton.addEventListener('click', () => this.switchToForm());
            }
        }
        ...
        loadSession() {
            const sessionId = this.getSessionId();
            const savedData = sessionStorage.getItem('aria_embed_session_' + sessionId);
        ...
        saveSession() {
            const sessionId = this.getSessionId();
            this.sessionData.conversationId = this.conversationId;
            sessionStorage.setItem('aria_embed_session_' + sessionId, JSON.stringify(this.sessionData));
        }
        ...
        getSessionId() {
            ...
            if (!sessionId) {
                sessionId = 'aria_embed_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
        ...
        handleIntakeSubmit(e) {
            ...
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (!emailRegex.test(email)) {
                this.showError(window.ariaChat ? window.ariaChat.strings.invalidEmail : 'Please enter a valid email address.');
                return;
            }
            ...
            this.startConversation(name, email, phone, message);
        }
        ...
        switchToChat() {
            ...
            this.chatView.offsetHeight;
            
            // Reapply theme to ensure all elements are updated
            if (this.currentTheme) {
                console.log('Reapplying theme:', this.currentTheme);
                this.applyTheme(this.currentTheme);
            } else {
                console.log('No current theme set, detecting...');
                this.detectTheme();
            }
        }
        ...
        startConversation(name, email, phone, initialMessage) {
            ...
            this.sendRequest('aria_send_message', {
                ...
                if (response.success) {
                    ...
                    this.addMessage(response.data.message || 'An error occurred. Please try again.', 'aria', 'error');
                }
            });
        }
         ...
         */
        scrollToBottom() {
            const scrollContainer = this.chatView.querySelector('.aria-embed-chat-messages');
            if (scrollContainer) {
        ...
        sendRequest(action, data, callback) {
            ...
            .catch(error => {
                ...
                this.hideTyping();
                this.addMessage('Connection error. Please try again.', 'aria', 'error');
            });
        }
        ...
        applyStyles() {
            ...
            let styleElement = document.getElementById('aria-embed-dynamic-styles');
            if (!styleElement) {
                styleElement = document.createElement('style');
        ...
        applyTheme(theme) {
            ...
            if (this.chatView) {
                this.chatView.classList.remove('aria-theme-light', 'aria-theme-dark');
         ...
         */
        getLastUserMessage() {
            const messages = this.messagesContainer.querySelectorAll('.aria-message-user');
            if (messages.length > 0) {
                const lastMessage = messages[messages.length - 1];
                const textElement = lastMessage.querySelector('.aria-message-content');
    ...
    function initAriaChat() {
        ...
        const style = document.createElement('style');
        const pluginUrl = chatConfig.pluginUrl || '/wp-content/plugins/aria/';
        style.textContent = `
            .aria-logo-light {
                background-image: url('${pluginUrl}public/images/aria.png') !important;
            }
        ...
        const embedContainers = document.querySelectorAll('[data-aria-embed="true"]');
